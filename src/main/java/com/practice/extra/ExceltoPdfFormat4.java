...
public class ExceltoPdfFormat4 {
	...
	public void generate(String srcFile, String destFile) {
		try {
			...
			Table table = new Table(new float[]{2,2,2,3,3,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2});
				...
				while (cellIterator.hasNext()) {
					...
					if(rowCount==1){
						if(cell.getStringCellValue().equals(""))
							continue;
						if(colCount==1)
							process(table,cell.getStringCellValue(), bold, 13, true);
						else{
							process(table,cell.getStringCellValue(), bold, 10, true);
						}
						continue;
					}else if(rowCount==2){
						...
						if(cell.getStringCellValue().equals(""))
							continue;
						if(colCount==1){
							process(table,cell.getStringCellValue(), bold, 5, true);
						}else
