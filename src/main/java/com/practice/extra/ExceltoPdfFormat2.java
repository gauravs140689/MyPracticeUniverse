...
public class ExceltoPdfFormat2 {
	...
	public void generate(String srcFile, String destFile) {
		try {
			...
			Table table = new Table(new float[]{2,3,2,3,3,3,3,3,3,3,3,3,3,3});
			...
			while (rowIterator.hasNext()) {
				...
				while (cellIterator.hasNext()) {
					...
					if(rowCount==1){
						process(table,cell.getStringCellValue(), bold, 14, true);
						break;
					}else if(rowCount==2){
						...
						if(colCount==1)
							...
							process(table,cell.getStringCellValue(), bold, 3, true);
						}
						continue;
					}
					...
					case Cell.CELL_TYPE_NUMERIC:
						if(colCount==1)
							continue;
						if(rowCount==3)
							process(table,cell.getNumericCellValue()+"", bold, 1, false);
						else
